...
class NavItem extends Component {
    ...
    render() {
        ...
        let navLinkURL = this.props.item.url;
        if (this.props.location.pathname.startsWith('/solutions/')) {
            let solutionId = this.props.location.pathname.split('/')[2]
            navLinkURL = navLinkURL.replace(':solutionId', solutionId);
        } else if (this.props.location.pathname.startsWith('/environments/')) {
            let environmentId = this.props.location.pathname.split('/')[2];
            navLinkURL = navLinkURL.replace(':environmentId', environmentId);
        }
        ...
        if(this.props.item.external) {
            subContent = (
                <a href={navLinkURL} target='_blank' rel='noopener noreferrer'>
            ...
            subContent = (
                <NavLink to={navLinkURL} className="nav-link" exact={true} target={itemTarget}>
                    <NavIcon items={this.props.item}/>
                    {itemTitle}
                    <NavBadge layout={this.props.layout} items={this.props.item}/>
                </NavLink>
