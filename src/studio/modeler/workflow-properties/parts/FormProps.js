...
module.exports = function (group, element, bpmnFactory, translate) {
    ...
    //         return {
    //             formKey: bo.get('custom:formKey')
    //         };
    //     },
    //     set: function (element, values, node) {
    //         var bo = getBusinessObject(element),
    //             formKey = values.formKey || undefined;
    //         return cmdHelper.updateBusinessObject(element, bo, { 'custom:formKey': formKey });
    //     }
    // }));

    function triggerCreateForm(element, entryNode, inputNode, inputName) {
        document.dispatchEvent(new CustomEvent('createForm', { detail: { eventSource: element } }));
    }
    
    var inputFormKey = entryFactory.selectBox({
        id: 'form-key',
        label: translate('Form Key'),
        modelProperty: 'formKey',
        selectOptions: function (element) {
            var options = [{ name: '', value: '' }]
            let currentValue = getBusinessObject(element).get('custom:formKey');
            if(currentValue) {
                let matchingForms = StudioConfig.INPUT_FORMS.filter(inputForm => inputForm.id === currentValue);
                if(matchingForms.length === 0) {
                    options.push({ name: translate(currentValue), value: currentValue })
                }
            }
            each(StudioConfig.INPUT_FORMS, function (inputForm) {
                options.push({ name: translate(inputForm.name), value: inputForm.id});
            });                
            return options;
        },
        get: function (element, node) {
            return { formKey: getBusinessObject(element).get('custom:formKey') };
        },
        set: function (element, values, node) {
            var formKey = values.formKey || undefined;
            return cmdHelper.updateBusinessObject(element, getBusinessObject(element), { 'custom:formKey': formKey });
        }
    });
    let customHtml = '<label for="camunda-form-key">Form Key</label>' +
                     '<button for="camunda-callable-element-ref" title="Create Form" data-show="isHidden" data-action="triggerCreateForm"' + 
                     ...
                     '<select id="camunda-form-key-select" name="formKey" data-value></select>';
    inputFormKey.html = customHtml;
    inputFormKey.triggerCreateForm = triggerCreateForm;
    group.entries.push(inputFormKey);
    ...
    var formFieldsEntry = extensionElements(element, bpmnFactory, {
        ...
        createExtensionElement: function (element, extensionElements, value) {
            ...
            if (!formData) {
                formData = elementHelper.createElement('custom:FormData', { fields: [] }, extensionElements, bpmnFactory);
            ...
            var field = elementHelper.createElement('custom:FormField', { id: value }, formData, bpmnFactory);
        ...
        removeExtensionElement: function (element, extensionElements, value, idx) {
            var formData = getExtensionElements(getBusinessObject(element), 'custom:FormData')[0],
    ...
    var formBusinessKeyFormFieldEntry = entryFactory.selectBox({
        ...
        get: function (element, node) {
            ...
            var bo = getBusinessObject(element);
            var formDataExtension = getExtensionElements(bo, 'custom:FormData');
    ...
    group.entries.push(entryFactory.comboBox({
        ...
        set: function (element, values, node) {
            var selectedFormField = getSelectedFormField(element, node),
                formData = getExtensionElements(getBusinessObject(element), 'custom:FormData')[0],
                commands = [];

            if (selectedFormField.type === 'enum' && values.type !== 'enum') {
                // delete dpie:value objects from formField.values when switching from type enum
    ...
    group.entries.push(entryFactory.table({
        ...
        addElement: function (element, node) {
            ...
            if (!validation) {
                ...
                validation = elementHelper.createElement('custom:Validation', {}, getBusinessObject(element), bpmnFactory);
        ...
        removeElement: function (element, node, idx) {
            ...
            if (constraints.length === 1) {
                // remove dpie:validation if the last existing constraint has been removed
    ...
    // [FormData] dpie:properties table
