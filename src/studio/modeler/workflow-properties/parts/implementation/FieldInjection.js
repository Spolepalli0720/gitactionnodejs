...
var FIELD_EXTENSION_ELEMENT = 'custom:Field';
...
module.exports = function (element, bpmnFactory, translate, options) {
    ...
    function getSelectedField(element, node) {
        ...
        var fields = getCustomFields(element, node);

        return fields[selected.idx];
    }

    function getCustomFields(element, node) {
        if (!insideListener) {
            return (
                businessObject &&
                extensionElementsHelper.getExtensionElements(businessObject, FIELD_EXTENSION_ELEMENT)
            ) || [];
        }
        return getCustomListenerFields(element, node);
    }

    function getCustomListenerFields(element, node) {
    ...
    var setOptionLabelValue = function () {
        return function (element, node, option, property, value, idx) {
            var customFields = getCustomFields(element, node);
    ...
    var newElement = function () {
        return function (element, extensionElements, value, node) {
            ...
            if (!insideListener) {
                ...
                newFieldElem = elementHelper.createElement(FIELD_EXTENSION_ELEMENT, props, extensionElements, bpmnFactory);
                return cmdHelper.addElementsTolist(element, extensionElements, 'values', [newFieldElem]);

            } else {

                var selectedListener = getSelectedListener(element, node);
                newFieldElem = elementHelper.createElement(FIELD_EXTENSION_ELEMENT, props, selectedListener, bpmnFactory);
    ...
    var fieldEntry = extensionElementsEntry(element, bpmnFactory, {
        ...
        getExtensionElements: function (element, node) {
            return getCustomFields(element, node);
        },
