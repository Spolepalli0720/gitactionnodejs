...
var EXECUTION_LISTENER_ELEMENT = 'custom:ExecutionListener';
var TASK_LISTENER_ELEMENT = 'custom:TaskListener';
...
module.exports = function (element, bpmnFactory, options, translate) {
    ...
    function getSelectedListener(element, node) {
        ...
        var listener = getListeners(bo, EXECUTION_LISTENER_ELEMENT)[selection.idx];
        if (!listener && taskListenerEntry) {
            selection = taskListenerEntry.getSelected(element, node);
            listener = getListeners(bo, TASK_LISTENER_ELEMENT)[selection.idx];
        }
        return listener;
    }
    ...
    if (is(element, 'bpmn:FlowElement') || is(element, 'bpmn:Process') || is(element, 'bpmn:Participant')) {
        ...
        if (bo) {
            ...
            var executionListenerEntry = extensionElementsEntry(element, bpmnFactory, {
                ...
                createExtensionElement: newElement(element, EXECUTION_LISTENER_ELEMENT, (isSequenceFlow) ? 'take' : 'start'),
                removeExtensionElement: removeElement(element, EXECUTION_LISTENER_ELEMENT),

                getExtensionElements: function (element) {
                    return getListeners(bo, EXECUTION_LISTENER_ELEMENT);
                },
                ...
                setOptionLabelValue: setOptionLabelValue(EXECUTION_LISTENER_ELEMENT)
    ...
    if (is(element, 'bpmn:UserTask')) {
        ...
        var taskListenerEntry = extensionElementsEntry(element, bpmnFactory, {
            ...
            createExtensionElement: newElement(element, TASK_LISTENER_ELEMENT, 'create'),
            removeExtensionElement: removeElement(element, TASK_LISTENER_ELEMENT),

            getExtensionElements: function (element) {
                return getListeners(bo, TASK_LISTENER_ELEMENT);
            },
            ...
            setOptionLabelValue: setOptionLabelValue(TASK_LISTENER_ELEMENT)
