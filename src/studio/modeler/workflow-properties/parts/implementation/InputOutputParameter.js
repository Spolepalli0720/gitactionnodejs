...
function isScript(elem) {
    return is(elem, 'custom:Script');
}

function isList(elem) {
    return is(elem, 'custom:List');
}

function isMap(elem) {
    return is(elem, 'custom:Map');
}
...
module.exports = function (element, bpmnFactory, options, translate) {
    ...
    entries.push(entryFactory.validationAwareTextField({
        ...
        disabled: function (element, node) {
            if (insideConnector && getSelected(element, node)) {
                if (getSelected(element, node).get('editable') !== undefined) {
                    return !getSelected(element, node).get('editable');
                } else {
                    return false;
                }
            } else {
    ...
    entries.push(entryFactory.selectBox({
        ...
        set: function (element, values, node) {
            ...
            if (parameterType === 'script') {
                properties.definition = createParameterTypeElem('custom:Script');
            }
            else if (parameterType === 'list') {
                properties.definition = createParameterTypeElem('custom:List');
            }
            else if (parameterType === 'map') {
                properties.definition = createParameterTypeElem('custom:Map');
            }
    ...
    entries.push(entryFactory.table({
        ...
        addElement: function (element, node) {
            var bo = getSelected(element, node);
            var newValue = createElement('custom:Value', bo.definition, bpmnFactory, { value: undefined });
    ...
    entries.push(entryFactory.table({
        ...
        addElement: function (element, node) {
            var bo = getSelected(element, node);
            var newEntry = createElement('custom:Entry', bo.definition, bpmnFactory, { key: undefined, value: undefined });
