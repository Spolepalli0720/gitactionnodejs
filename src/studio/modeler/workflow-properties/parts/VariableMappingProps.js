...
/**
  * return depend on parameter 'type' dpie:in or dpie:out extension elements
  */
function getInOutMappings(element, type) {
...
function getVariableMappings(element, type) {
    var variableMappings = getInOutMappings(element, type);

    return filter(variableMappings, function (mapping) {
...
var EXTENSION_ELEMENT_IN = 'custom:In',
    EXTENSION_ELEMENT_OUT = 'custom:Out';
...
module.exports = function (group, element, bpmnFactory, translate) {
    ...
    if (!is(element, 'custom:CallActivity') && !signalEventDefinition) {
        return;
    }
    ...
    var getSelected = function (element, node) {
        ...
        var parameter = getVariableMappings(element, EXTENSION_ELEMENT_IN)[selection.idx];

        if (!parameter && outEntry) {
            selection = outEntry.getSelected(element, parentNode);
            parameter = getVariableMappings(element, EXTENSION_ELEMENT_OUT)[selection.idx];
        }
    ...
    var inEntry = extensionElementsEntry(element, bpmnFactory, {
        ...
        createExtensionElement: newElement(EXTENSION_ELEMENT_IN),
        removeExtensionElement: removeElement(EXTENSION_ELEMENT_IN),

        getExtensionElements: function (element) {
            return getVariableMappings(element, EXTENSION_ELEMENT_IN);
        },
        ...
        setOptionLabelValue: setOptionLabelValue(EXTENSION_ELEMENT_IN)
    });
    ...
    if (!signalEventDefinition) {
        var outEntry = extensionElementsEntry(element, bpmnFactory, {
            ...
            createExtensionElement: newElement(EXTENSION_ELEMENT_OUT),
            removeExtensionElement: removeElement(EXTENSION_ELEMENT_OUT),

            getExtensionElements: function (element) {
                return getVariableMappings(element, EXTENSION_ELEMENT_OUT);
            },
            ...
            setOptionLabelValue: setOptionLabelValue(EXTENSION_ELEMENT_OUT)
        });
    ...
    group.entries.push(entryFactory.label({
        ...
        get: function (element, node) {
            ...
            var value = '';
            if (is(mapping, EXTENSION_ELEMENT_IN)) {
                value = translate('In Mapping');
            }
            else if (is(mapping, EXTENSION_ELEMENT_OUT)) {
