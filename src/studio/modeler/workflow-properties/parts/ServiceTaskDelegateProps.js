...
module.exports = function (group, element, bpmnFactory, translate) {
    ...
    group.entries.push(entryFactory.link({
        ...
        handleClick: function (element, node, event) {
            ...
            var connectorTabEl = getTabNode(node, 'propertiesTab');
        ...
        showLink: function (element, node) {
            ...
            if (isConnector(element)) {
                ...
                return true;
            } else if(element.businessObject && element.businessObject.get("decisionRef") !== undefined) {
                if(element.businessObject.get("decisionRef") === '') {
                    link.innerHTML = '<span class="bpp-icon-warning"></span> ' + escapeHTML(translate('Must configure Decision Rules'));
                    domClasses(link).add('bpp-error-message');
                } else {
                    link.textContent = translate('Configure Decision Rule');
                }
