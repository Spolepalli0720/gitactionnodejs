...
import { inputField, actionButton } from './StudioUtils';


export default class StudioShare extends Component {
    ...
    componentDidMount() {
        ...
        userService.getUsers().then(response => {
            ...
            users = response.filter(user => user.verified).sort((a, b) => a.displayName.localeCompare(b.displayName));
    ...
    renderUser(user, shareGroup) {
        ...
        return (
            <Row>
                ...
                {shareGroup &&
                    <Col sm='auto'>
                        {shareGroup && user.username !== userService.getLoginName() &&
                            actionButton('Remove Sharing', this.removeShare.bind(this, user.username, shareGroup),
                                'mt-0 ml-0 mr-3 content-float-right align-top', 'feather icon-x-circle text-danger',
                                false, user.username === userService.getLoginName())
                        }
                        {user.username === userService.getLoginName() &&
                            <label className='mt-1 mr-3 mb-0'>Owner</label>
                        }
                        {user.username !== userService.getLoginName() && inputField('select', shareGroup, '',
                            shareGroup === 'sharedUser' ? sharedUsers[user.username] : selectedUsers[user.username],
                            this.onChangeRole.bind(this, user.username), {
                            container: 'content-float-right',
                            input: 'w-auto content-border-none text-right'
                        }, roles)
                        }
                    </Col>
                }
